-- 020_farkle_seed_isolation_hotfix.sql
--
-- Migration 018 inadvertently restored the metadata.seed exposure that 014 removed:
-- its INSERT used jsonb_build_object('seed', p_seed, 'modeKey', p_mode_key) and
-- omitted the server_seed column.  This migration re-creates farkle_enter_match
-- with both fixes applied together:
--   · p_invite_code parameter  (from 018)
--   · seed written to server_seed column, NOT to metadata  (from 014)
--   · metadata only carries modeKey (non-sensitive)
...
begin
  ...
  -- ── Create match: seed in server_seed column only; metadata is non-sensitive ──
