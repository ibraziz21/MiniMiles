...
-- App/backend table only; the actual owner link is confirmed on-chain by
-- SuperChainModule.populateAddOwner + addOwnerWithThreshold.
...
-- Service-role clients bypass RLS. The app APIs are responsible for session
-- checks, signature verification, and on-chain transaction verification.
