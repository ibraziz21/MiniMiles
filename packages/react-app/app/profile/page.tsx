  ...
  return (
    <div className={`rounded-2xl p-4 flex flex-col items-start gap-3 sm:flex-row sm:items-center
      ...
      {claimed ? (
        ...
        >
          {loading ? '…' : unlocked ? 'Claim' : `${pct - completion}% to go`}
        </button>
      )}
    </div>
  );
...
export default function ProfilePage() {
  ...
  const claimMilestone = async (milestone: 50 | 100) => {
    if (!address) return;
    const res = await fetch('/api/profile/claim-milestone', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userAddress: address, milestone }),
    });
    const json = await res.json();
    if (!res.ok) {
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mx-4 mt-4 bg-white rounded-2xl px-4 pt-1 pb-1 shadow-sm border border-gray-100">
            ...
            <EditRow label="Country" value={profile?.country ?? ''} placeholder="e.g. Kenya" onSave={v => patchField('country', v)} />
