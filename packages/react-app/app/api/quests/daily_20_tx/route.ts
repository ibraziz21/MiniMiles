/* ────────────────────────────────────────────────────────────────────────────
   ...
   const SUBGRAPH_USDT = "https://api.studio.thegraph.com/query/114722/transfers-6-d/version/latest"  // https://api.studio…thegraph.com/query/…/transfers-6-d/…
   if (
     !SUPABASE_URL ||
     !SUPABASE_SERVICE_KEY ||
     !PRIVATE_KEY ||
   ...
   async function countTransfersIn24H(user: string): Promise<number> {
     ...
     for (const url of urls) {
       ...
       if (total >= 20) return total; // early exit
     }
     return total;
   }
     ...
     try {
       ...
       if (txs < 20)
         return NextResponse.json({
           success: false,
           message: `Only ${txs}/20 transfers in the last 24 h`,
         });
