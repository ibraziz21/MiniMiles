...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const action = isFarkle ? "farkle" : isFirstRoll ? "roll" : "roll_again";
  console.log(
    `[farkle/roll] matchId=${matchId} wallet=${address} turn=${match.turn_number}` +
      ` roll=${rollNumber} action=${action} turnPoints=${newTurnPoints} isFarkle=${isFarkle}` +
      ` hotDice=${triggeredHotDice}`,
  );
