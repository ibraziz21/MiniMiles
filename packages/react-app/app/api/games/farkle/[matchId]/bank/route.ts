...
export async function POST(req: Request, { params }: Ctx) {
  ...
  if (newBanked >= targetScore) {
    ...
    try {
      await settle(match, address, opp.wallet_address, newBanked, oppScore, players ?? []);
    } catch (err) {
      console.error("[farkle/bank] failed to settle match", err);
...
async function settle(match: any, winnerAddress: string, loserAddress: string, winnerScore: number, loserScore: number, players: any[]) {
  ...
  const { error: settleMatchError } = await supabase.from("game_matches").update({
    ...
    revealed_seed: match.metadata?.seed, completed_at: new Date().toISOString(),
  }).eq("id", match.id);
  ...
  // Grant rewards + mint AkibaMiles on-chain
