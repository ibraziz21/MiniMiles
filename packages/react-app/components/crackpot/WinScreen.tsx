...
export function WinScreen({ potWon, potWonUsdt, totalGuesses, theme, version, cycleId, onClose }: WinScreenProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-end sm:items-center justify-center overflow-hidden transition-all duration-400 ${visible ? "opacity-100" : "opacity-0"}`}
    ...
    >
      <motion.div
        ...
        className="w-full max-w-sm rounded-t-3xl sm:rounded-3xl bg-white p-5 text-center shadow-2xl max-h-[92dvh] overflow-y-auto"
      >
        {/* Explosion */}
        <motion.div
        ...
        <div className="text-3xl mb-2">{theme.symbols.slice(0, 4).join(" ")}</div>

        <h2 className="text-2xl font-black text-slate-900 mb-0.5">YOU CRACKED IT!</h2>
        ...
        >
          <TokenAmount
            ...
            gap="gap-2"
          />
          <div className="text-sm font-medium text-slate-500 mt-1.5">won</div>
        </motion.div>
