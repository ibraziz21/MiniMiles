...
//
// Keyed off the canonical `playerKey`/`displayName`/`isYou` contract
// (skill-games-leaderboards-spec.md §4.2) rather than a raw wallet address —
// GET /api/games/leaderboard resolves the display name (an `@username` or a
// neutral alias) and the viewer match server-side now, so this never sees a
// wallet.
...
export function avatarBg(key: string) {
  const palette = [
...
export function EntryRow({
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 ${isYou ? "bg-[#F0FDFF]" : ""}`}>
      ...
      <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold ${avatarBg(entry.playerKey)}`}>
        {initials(entry.displayName)}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <p className="text-sm font-semibold text-[#1A1A1A] truncate">{entry.displayName}</p>
          {isYou && (
