...
export function IntentShortcuts({ intents, title }: { intents: DiscoveryIntent[]; title: string }) {
  ...
  return (
    <section className="mb-5">
      <h2 className="mb-2.5 text-sm font-semibold uppercase tracking-wide text-akiba-muted">{title}</h2>
        ...
        {intents.map((intent, i) => {
          ...
          return (
            <TrackedLink
              ...
              className="flex shrink-0 items-center gap-2 rounded-full border border-akiba-line bg-white px-4 py-2.5 text-sm font-medium text-akiba-ink transition hover:border-akiba-teal/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-akiba-teal"
            >
