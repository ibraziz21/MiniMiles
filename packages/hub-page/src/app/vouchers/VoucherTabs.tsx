...
import { Tag, ExternalLink, ShoppingBag, Smartphone, QrCode, ChevronRight, Loader2 } from "lucide-react";
...
// Maps HTTP status codes from POST /api/shop/vouchers/redeem to user-friendly messages.
...
export function VoucherTabs({
  ...
  return (
    <div>
      <div className="mb-6 flex gap-1 rounded-2xl bg-akiba-card p-1">
        {(["available", "active", "used", "expired"] as Tab[]).map((tn) => (
          <button
            ...
            className={clsx(
              ...
              tab === tn ? "bg-white text-akiba-ink shadow-chip" : "text-akiba-muted hover:text-akiba-ink"
            )}
          >
            {TAB_LABELS[tn]}
          </button>
      ...
      {tab === "available" &&
        (templates.length === 0 ? (
          <EmptyState icon={<Tag className="mb-3 h-10 w-10 text-akiba-line" />} title="No vouchers available yet" subtitle="Check back as merchants add new offers." />
        ) : (
      ...
      {needsMine &&
        (!isSignedIn ? (
          <div className="flex flex-col items-center rounded-2xl border border-dashed border-akiba-line bg-white py-14 text-center">
            ...
            <a href="/login" className="mt-4 rounded-full bg-akiba-teal px-5 py-2 text-sm font-semibold text-white">
              Sign in
            </a>
          </div>
        ) : loading ? (
          ...
          <EmptyState
            ...
            title={
              tab === "active" ? "No active vouchers" : tab === "used" ? "No used vouchers yet" : "No expired vouchers"
            }
...
function AvailableCard({
  ...
  async function handleRedeem() {
    ...
    try {
      ...
      // Navigate to the issued voucher page.
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-akiba-line bg-white">
      <div className="flex items-center gap-3 border-b border-dashed border-akiba-line bg-akiba-tint px-4 py-3">
        ...
        <div>
          ...
          <p className="font-sterling text-lg font-bold text-akiba-teal">{discountLabel(t)}</p>
        </div>
      </div>

      <div className="flex flex-1 flex-col space-y-1 p-4 text-sm text-akiba-muted">
        <p className="font-medium text-akiba-ink">{t.title}</p>
        {t.applicable_category && (
          <p>On: <span className="font-medium capitalize text-akiba-ink">{t.applicable_category}</span></p>
        )}
      </div>
      ...
      {merchant && (
        <a
          ...
          className="flex items-center justify-center gap-1.5 border-t border-akiba-line bg-akiba-card py-2.5 text-xs font-semibold text-akiba-teal transition hover:bg-akiba-tint"
        >
...
/**
 * Branded digital card. No raw code is shown — the card links to the voucher
 * detail page where the user generates a rotating QR for in-store redemption.
 */
function IssuedCard({ voucher: v }: { voucher: IssuedVoucher }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-dashed border-akiba-line bg-akiba-tint px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-akiba-muted">{merchant?.name ?? "All merchants"}</p>
            <p className="font-sterling text-lg font-bold text-akiba-teal">
              {t ? discountLabel(t) : "Voucher"}
            </p>
          </div>
        </div>
        <span className={clsx("rounded-full px-2.5 py-0.5 text-[11px] font-semibold capitalize", statusColors[v.status])}>
          {v.status}
        </span>
      </div>

      <div className="space-y-0.5 px-4 py-4 text-xs text-akiba-muted">
        {t && <p className="text-sm font-medium text-akiba-ink">{t.title}</p>}
        {v.expires_at && <p>Expires: {new Date(v.expires_at).toLocaleDateString()}</p>}
        {v.redeemed_at && <p>Used: {new Date(v.redeemed_at).toLocaleDateString()}</p>}
      ...
      <div className="mt-auto flex items-center justify-between border-t border-akiba-line bg-akiba-card px-4 py-2.5 text-xs font-semibold text-akiba-teal">
        <span className="flex items-center gap-1.5">
          {v.status === "issued" ? <><QrCode className="h-3.5 w-3.5" /> Show QR to redeem</> : "View details"}
        </span>
