...
export async function POST(req: Request) {
  const auth = await requireGameIdentity(
    req,
    { scope: "games_finish", limit: 30, windowSeconds: 600 },
    { mutation: true }
  );
  ...
  try {
    const result = await gamesBackend.finish(auth.context.identity, body.sessionId);
    return NextResponse.json(result);
  } catch (err) {
    if (err instanceof GamesBackendError) {
      return NextResponse.json({ error: err.code }, { status: err.status === 401 ? 503 : err.status });
    }
    console.error("[api/games/session/finish] backend call failed:", err);
