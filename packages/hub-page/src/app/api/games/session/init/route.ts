...
export async function POST(req: Request) {
  const auth = await requireGameIdentity(
    req,
    { scope: "games_init", limit: 30, windowSeconds: 600 },
    { mutation: true }
  );
  ...
  try {
    const result = await gamesBackend.init(auth.context.identity, body.sessionId);
    return NextResponse.json(result);
  } catch (err) {
    if (err instanceof GamesBackendError) {
      return NextResponse.json({ error: err.code }, { status: err.status === 401 ? 503 : err.status });
    }
    console.error("[api/games/session/init] backend call failed:", err);
