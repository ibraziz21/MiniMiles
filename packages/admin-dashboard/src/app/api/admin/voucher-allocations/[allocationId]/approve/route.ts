...
// POST /api/admin/voucher-allocations/:allocationId/approve
export async function POST(
  ...
  const { data, error } = await transitionAllocation(allocationId, "approve", actorId, reason);
  if (error) return rpcErrorResponse(error);

  await writeAdminAuditLog({
    adminUserId: adminIdForWrite(session),
    action: "voucher_allocation.approved",
