...
// POST /api/admin/voucher-allocations/:allocationId/end — permanently stops issuance.
export async function POST(
  ...
  const { data, error } = await transitionAllocation(allocationId, "end", actorId, reason);
  if (error) return rpcErrorResponse(error);

  await writeAdminAuditLog({
    adminUserId: adminIdForWrite(session),
    action: "voucher_allocation.ended",
