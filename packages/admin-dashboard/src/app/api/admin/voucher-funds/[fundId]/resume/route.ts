...
// POST /api/admin/voucher-funds/:fundId/resume — resume claims after a pause.
export async function POST(req: NextRequest, { params }: { params: Promise<{ fundId: string }> }) {
  ...
  const { data, error } = await transitionProgram(fundId, "resume", actorId, reason, null);
  if (error) return rpcErrorResponse(error);

  await writeAdminAuditLog({
    adminUserId: adminIdForWrite(session),
    action: "voucher_fund.resumed",
