...
export function AllocationActions({
  ...
  async function act(action: string, body: Record<string, unknown> = {}) {
    setBusy(true);
    setError(null);
    const res = await fetch(`/api/admin/voucher-allocations/${allocationId}/${action}`, {
      method: "POST",
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {state === "draft" && canWrite && (
          <>
            <Button asChild variant="outline" size="sm">
              <Link href={`/vouchers/funds/${fundId}/allocations/${allocationId}/edit`}>Edit</Link>
            </Button>
        ...
        {state === "pending_approval" && canApprove && (
          <>
            <Button
              ...
              disabled={busy}
              onClick={() => {
                const reason = prompt("Approval reason (optional)") ?? undefined;
        ...
        {(state === "scheduled" || state === "active") && canPublish && (
          <>
            ...
            <Button
              ...
              disabled={busy}
              onClick={() => {
                if (!confirm("End this allocation permanently? Issued vouchers remain valid.")) return;
        ...
        {(state === "paused" || state === "exhausted") && canPublish && (
          <>
            {state === "paused" && (
