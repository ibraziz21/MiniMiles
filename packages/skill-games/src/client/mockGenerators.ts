// Local (non-authoritative) board generators used only for mock-mode play —
// i.e. when a host has no server transport wired up (dev without a
// configured contract). Never used to produce an accepted result; the
// authoritative score always comes from the host's transport/finish call.

import type { RuleTapRule, RuleTapTile } from "../core/types";

type Rng = () => number;

// FNV-1a 32-bit — used only for the game RNG, not for any commitment.
