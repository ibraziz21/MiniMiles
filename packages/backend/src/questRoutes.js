...
const questChecks_1 = require("./questChecks");
...
router.post("/25tx", async (req, res) => {
    try {
        ...
        // 2) Check chain logic
        const eligible = await (0, questChecks_1.has25Transactions)(userAddress);
        if (!eligible) {
            return res.status(400).json({ error: "Not eligible (not 25 tx yet)." });
        }
        // 3) Mint points
        const pointsToMint = 20n; // or BigInt(20)
        await (0, questChecks_1.mintMiniPoints)(userAddress, pointsToMint);
        // 4) Insert record
        const { data: insertData, error: insertError } = await supabaseClient_1.supabase
            .from("claims")
...
router.post("/transfer5usdt", async (req, res) => {
    try {
        ...
        // 2) Check chain logic
        const eligible = await (0, questChecks_1.hasTransferred5USDT)(userAddress);
        if (!eligible) {
